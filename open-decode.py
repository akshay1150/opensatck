...
@app.route("/create_stack",methods=['GET', 'POST'])
def create_stack():
    ...
    yaml_file_in_dict = yaml.load(template)
    # x = yaml.dump(template)
    #parameters=None
#    template = open(stack_file_path)
    # template=open(yaml_file)
    # if parameters:
    #     stack = heatclient.stacks.create(stack_name=stack_name,
    #     template=template.read(), parameters=parameters)
    # else:
    #print(template.read())
    stack = heatclient.stacks.create(stack_name=stack_name,template=yaml.dump(yaml_file_in_dict))
    # template.close()
#    return stack
    return stack 
